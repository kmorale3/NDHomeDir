...
int main(int argc, char *argv[]) {
    ...
    // check that the correct number of arguments were given, otherwise return failure
    ...
    // create the target file and give the user read, write, execute permissions, creates a file descriptor
    ...
    while (bytes_read > 0) {
        ...
        if (bytes_written != bytes_read){
            printf("filecopy: Error: %ld bytes just read, but %ld bytes just written\n",bytes_read,bytes_written);
            return EXIT_FAILURE;
        }
