...
def count_letters(string):
    ...
    for letter in string:
        if not letter.isspace(): #checks to make sure that letter is not a space  
            #look p letter in counts dictionary
            #   if letter is in count, use the existing val
            #   otherwise, if letter is not in count, use existing val
